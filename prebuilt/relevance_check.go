...
// relevanceCheckPromptInput is the named template substitution struct for [relevanceCheckUserPrompt].
...
func NewRelevanceCheckAgent(chatModel model.ToolCallingChatModel, opts ...RelevanceCheckOption) (*RelevanceCheckAgent, error) {
	...
	systemPrompt := relevanceCheckSystemPrompt
	if cfg.SystemPrompt != "" {
		systemPrompt = cfg.SystemPrompt + "\n\n" + relevanceCheckSystemPrompt
	}
...
// relevanceCheckSystemPrompt is the static system prompt: role, rules, score scale, examples.
const relevanceCheckSystemPrompt = `You are an expert judge. Rate how directly and completely the LLM response addresses the user question.
...
const relevanceCheckUserPrompt = `<user_question>
${Question}
</user_question>
